    ...
    $(document).ready(function(){
		...
		var ut_word_rotator = function() {
			...
			var ut_rotator_words = [
				'We are <span>Artists </span>',
				'We love to <span>Create</span>',
				'We love <span>Liège</span>'
			] ,
			counter = 0;                
		...
		if( !device.tablet() && !device.mobile() ) {
			...
			$('#parallax-section-3').addClass('parallax-banner-3');
		...
		$("#contact-form").submit(function(){
			var myModel = Ext.create(dcodeGenForm.model.dcodegenform,{});
			$(this).find('[placeholder]').each(function() {
				var input = $(this);
				if (input.val() == input.attr('placeholder')) {
					input.val('');
				};
                myModel.set(input.attr('name'),input.val());
			});
			var data = myModel.save();
		...
		function animate_sections() {
			...
			$('#service-section').waypoint( function( direction ) {
				...
				if( direction === 'down' && !$(this).hasClass('animated') ) {
					...
					setTimeout ( function () {
							$('#service-section header').find('.section-title').animate({ opacity: 1 });
					}, 200 );

					setTimeout ( function () {
							$('#service-section header').find('.section-slogan').animate({ opacity: 1 });
					}, 600 );
